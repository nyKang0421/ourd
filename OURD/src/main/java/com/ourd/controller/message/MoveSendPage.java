...
public class MoveSendPage implements Controller {
	...
	public String requestHandler(HttpServletRequest request, HttpServletResponse response)
		...
		//System.out.println(request.getParameter("move").getClass().getSimpleName());
		HttpSession session = request.getSession();
		int num = -1;
		if(move.equals("post")) {
			num = (int) session.getAttribute("sendPage") +1;
			System.out.println("nextPage:"+num);
		}else if(move.equals("pre")) {
			num = (int) session.getAttribute("sendPage") -1;
			System.out.println("nextPage:"+num);
		}
		session.setAttribute("sendPage", num);
